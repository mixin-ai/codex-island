...
**Goal:** Keep usage on the left and show up to two local Codex tasks, including their execution state, on the right of the notch.

**Architecture:** A read-only SQLite query supplies task identities, names, and rollout paths. A background scanner reads lifecycle events from those rollouts and publishes a maximum of two tasks, prioritizing running tasks. The compact notch and expanded Usage page share the same task list. A separate glow preference disables every decorative outer glow.

**Tech Stack:** Swift, SwiftUI, Combine, SQLite3, Foundation.

## Design decisions

- Prefer an always-visible two-row sidebar over a rotating single task or an expanded-only list, so concurrent tasks can be checked at a glance.
...
- Keep the physical camera area clear and preserve the upstream compact/peek width and original notch height. Limit compact task names to six characters plus an ellipsis and expose the full name via hover and accessibility.
...
3. Add `Sources/Views/CodexTaskList.swift`; integrate it into `IslandRootView.swift`, `UsageView.swift`, and `PanelHeader.swift`. Keep the original notch dimensions and fit two short task rows into the existing right side.
...
- At most two task names appear on the right; the current active task is marked 执行中.
